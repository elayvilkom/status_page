...
def multivalue_field_factory(field_class):
    """
    Given a form field class, return a subclass capable of accepting multiple values.yaml. This allows us to OR on multiple
    filter values.yaml while maintaining the field's built-in validation. Example: GET /api/dcim/devices/?name=foo&name=bar
    """
    class NewField(field_class):
        ...
        def to_python(self, value):
            ...
            field = field_class()
            return [
                # Only append non-empty values.yaml (this avoids e.g. trying to cast '' as an integer)
